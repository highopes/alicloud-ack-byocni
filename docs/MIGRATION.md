...
- 应用镜像来源继续固定为 `https://github.com/highopes/galileo-demo` commit `26ea7c0c3276cde94b6f5c1bff2b0d741d655858` 对应、已完成 smoke/push 的 `linux/amd64` ACR immutable digest；`kup` 不在 lifecycle 中临时 clone 或 build。上游 commit `4ce9a4d24adc3bf91783cd68383a79b63e46cad4` 新增的 Qwen baseline 只包含 Prompt 变更，本仓库复制其精确文本并通过运行时 `custom` profile 加载，没有伪造新的镜像来源或使用 mutable tag。
...
- 新增硬检查：Deployment 必须使用配置的 `@sha256` digest；Prompt ConfigMap 键、projected volume/env 路径与应用 resolver 必须收敛到最终语义选择；Qwen baseline 还必须与 `ns_galileo/supervisor-baseline-qwen.txt` 做 SHA-256 一致性校验。Service 必须有 endpoint，testcurl 必须能访问 Chainlit HTTP；Pod 内上游 `pod_network_smoke.py` 必须验证应用模型 DNS/TLS/认证与精确 model ID、Pinecone integrated text search 和 Splunk AO HTTPS。
- 新增 `galileo-prompt` 操作入口、`ns_galileo/supervisor-baseline-qwen.txt` 和 `ns_galileo/supervisor-production-example.txt`。脚本以 `official-baseline`、`qwen-baseline`、`improved` 三个命令明确切换演示提示词，保留 `baseline` 作为官方 baseline 的兼容别名，并继续支持任意 UTF-8 `custom` 文件；使用 0600 临时 JSON merge patch，等待 projected ConfigMap 与 resolver 同步，并要求 Pod UID、restart count 和 image ID 在切换前后完全不变。切回 built-in profile 时会清空 custom 内容，`status` 通过内容哈希识别 Qwen 热补丁。
...
- Qwen baseline 热补丁升级状态：static passed。按用户要求未重跑 `kup` 或触碰已正确安装的 ACK 应用；本次仅同步上游 Prompt、增加默认选择变量和安装后自动热加载，并完成 shell、Terraform format、模板/YAML、Prompt 内容哈希与 Git diff 静态检查。先前真实 lifecycle 与 prompt 热切换证据保持有效，但不把本次静态验证误写成新的集群回归。
