...
- 应用镜像来源继续固定为 `https://github.com/highopes/galileo-demo` commit `26ea7c0c3276cde94b6f5c1bff2b0d741d655858` 对应、已完成 smoke/push 的 `linux/amd64` ACR immutable digest；`kup` 不在 lifecycle 中临时 clone 或 build。上游 commit `4ce9a4d24adc3bf91783cd68383a79b63e46cad4` 没有要求替换当前镜像；本仓库以相同的 `app/prompts/supervisor-baseline-qwen.txt` 路径和运行时 `custom` profile 加载其 Qwen baseline，没有伪造新的镜像来源或使用 mutable tag。
...
- `kup.conf.example` 增加完整 `GALILEO_*` 接口，敏感字段只有 `ReplaceMe`；真实 ACR、Splunk AO、百炼、Pinecone 值已从获授权的本地 Galileo `.secrets`/resolved runtime 写入 ignored、0600 的 `kup.conf`。新增 `GALILEO_BASELINE_PROMPT_VARIANT=official|qwen`，默认 `qwen`；当 Fresh profile 是 baseline 时，`kup` 在 Pod Ready 后将仓库附带的 Qwen 文本作为 `custom` ConfigMap 热补丁加载。VLLM Judge key、Docker Hub PAT 与 Alibaba RAM credential 不注入应用 Pod。
- `kup` 使用临时 0600 文件生成 runtime Secret 与 Docker config Secret，apply 后通过 trap 立即清理；Terraform/命令输出脱敏器也覆盖新增凭据。
- 新增硬检查：Deployment 必须使用配置的 `@sha256` digest；Prompt ConfigMap 键、projected volume/env 路径与应用 resolver 必须收敛到最终语义选择；Qwen baseline 还必须与 `app/prompts/supervisor-baseline-qwen.txt` 做 SHA-256 一致性校验。Service 必须有 endpoint，testcurl 必须能访问 Chainlit HTTP；Pod 内上游 `pod_network_smoke.py` 必须验证应用模型 DNS/TLS/认证与精确 model ID、Pinecone integrated text search 和 Splunk AO HTTPS。
- Prompt 运维入口现与上游绝对对齐：本仓库的 `scripts/switch_prompt.sh`、`scripts/render_prompt_patch.py`、`app/prompts/supervisor-baseline-qwen.txt` 和 `app/prompts/supervisor-production-example.txt` 均与 Galileo commit `4ce9a4d24adc3bf91783cd68383a79b63e46cad4` 对应文件逐字节一致。只使用上游定义的 `status | baseline | improved | custom FILE`；Qwen baseline 也严格使用 `custom app/prompts/supervisor-baseline-qwen.txt`，不再维护 ACK 专用 alias 或第二套 Prompt 路径。ACK 侧仅通过上游原生支持的 `KUBECONFIG_FILE`、`KUBE_CONTEXT`、`KUBE_NAMESPACE` 指向同一集群和 namespace。
...
- Qwen baseline 热补丁升级状态：static passed。按用户要求未重跑 `kup` 或触碰已正确安装的 ACK 应用；本次同步上游 Prompt、增加默认选择变量、让 `kup` 通过上游原版脚本自动热加载，并完成 shell、Python、Terraform format、模板/YAML、四个上游文件逐字节一致性与 Git diff 静态检查。被替换的 ACK 专用 `galileo-prompt` 和旧 `ns_galileo/` Prompt 路径已备份到 `.bak/20260914-galileo-prompt-alignment/` 后删除，避免同一应用出现两套运维入口。先前真实 lifecycle 与 prompt 热切换证据保持有效，但不把本次静态验证误写成新的集群回归。
